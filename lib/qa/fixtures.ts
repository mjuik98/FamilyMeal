...
const getDayKey = (date: Date) =>
  `${date.getFullYear()}-${`${date.getMonth() + 1}`.padStart(2, "0")}-${`${date
...
const createQaArchiveBacklogMeals = (
  role: UserRole,
  referenceDate: Date
): Meal[] => {
      ...
      comments: [
        {
          ...
          timestamp: dinner + 2 * 60 * 60 * 1000 + 30_000,
        },
        {
          ...
          timestamp: dinner + 2 * 60 * 60 * 1000 + 60_000,
        },
        {
          ...
          timestamp: dinner + 2 * 60 * 60 * 1000 + 90_000,
        },
      ],
    },
  ];
  ...
  return rawMeals
    .map((meal): Meal | null => {
      ...
      return {
        ...
        reactions:
          typeof raw.reactions === "object" && raw.reactions ? raw.reactions : {},
      };
    })
...
export const createQaMockWeekMeals = (
  ...
  const customMeals = normalizeQaCustomMeals(
    readJsonFromStorage(QA_CUSTOM_MEALS_KEY, [])
  ).filter(
    (meal) =>
      meal.timestamp >= weekStart.getTime() && meal.timestamp <= weekEnd.getTime()
  );

  return [...todayMeals, ...historicMeals, ...customMeals].sort(
...
export const createQaMockMeals = (
  ...
  createQaMockWeekMeals(role, date, focalDate).filter((meal) =>
    isSameDay(new Date(meal.timestamp), date)
  );
...
export const getQaMockMealById = (
  ...
  createQaMockRecentMeals(role, referenceDate, focalDate).find(
    (meal) => meal.id === mealId
  ) ?? null;
...
export const createQaMockWeeklyStats = (
  ...
  return QA_WEEKLY_COUNTS.map((_, index) => {
    ...
    const preview = meals.find(
      (meal) => isSameDay(new Date(meal.timestamp), date) && meal.imageUrl
    );
